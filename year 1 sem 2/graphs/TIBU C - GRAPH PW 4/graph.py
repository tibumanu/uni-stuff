...
class DirectedGraph:
    ...
    def addEdge(self, node1, node2, weight):
        ...
        if self.checkEdge(node1, node2):
            ...
            self.nodesIn[node1].append(node2)
            self.nodesOut[node2].append(node1)
    ...
    def cheapestNeighbour(self, node):
        neighbours = self.parseIn(node)
        cheapestCost = 99999
        cheapestNode = -1
        for each in neighbours:
            if self.edges[(node, each)] < cheapestCost:
                cheapestNode = each
                cheapestCost = self.edges[(node, each)]
