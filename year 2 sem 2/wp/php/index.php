<?php $title='Home | Dumb News'; include("template/top.php");?>

<?php include("component/genericheader.php");?>

<main class="w-5/6 h-full mx-auto flex justify-center flex-col">
<?php
    if (isset($_SESSION['DumbNews_loggedin']) && $_SESSION['DumbNews_loggedin'] == true) {
        if(!isset($_POST["category"]) && !isset($_POST["date-start"]) && !isset($_POST["date-end"]))
        {
            echo "<div class='flex justify-center items-center w-full h-12 bg-slate-900 shadow-sm rounded-t-md text-white text-lg font-bold'>Welcome, " . $_SESSION['DumbNews_username'] . " let's have a look at the news:" . "</div>";
