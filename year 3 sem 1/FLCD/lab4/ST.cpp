...
Node* SymbolTable::getRootIdentifiers() const{
    return rootIdentifiers;
}

Node* SymbolTable::getRootConstants() const{
    return rootConstants;
}

Node* SymbolTable::putIdentifier(std::string key){
    if(containsIdentifier(key)) return getIdentifier(rootIdentifiers, key);
...
Node* SymbolTable::putConstant(std::string key){
    if(containsConstant(key)) return getConstant(rootConstants, key);
...
std::vector<std::pair<std::string, int>> SymbolTable::getTableOfIdentifiers()
{
    std::vector<std::pair<std::string, int>> v;
    std::queue<Node*> q;
    q.push(rootIdentifiers);
    while(!q.empty()){
        Node* current = q.front();
        q.pop();
        v.push_back(std::make_pair(current->key, current->index));
        if(current->left != NULL)   q.push(current->left);
        if(current->right != NULL)  q.push(current->right);
    }
    return v;
...
std::vector<std::pair<std::string, int>> SymbolTable::getTableOfConstants()
{
    std::vector<std::pair<std::string, int>> v;
    std::queue<Node*> q;
    q.push(rootConstants);
    while(!q.empty()){
        Node* current = q.front();
        q.pop();
